...
app.post('/details', upload.single('picture'), function (req, res, next) {
    cloudinary.uploader.upload(req.file.path, function (error, result) {
        ...
        db.collection('books').insertOne(data, function (error, result) {
            if (error) throw error
            res.send("Inserted")
        })
    });
...
app.put('/updatebook', function (req, res) {
    db.collection('books').updateOne({ '_id': require('mongodb').ObjectID(req.query.id) }, { $set: { "name": req.query.name } }, function (error, result) {
        if (error) throw (error)
        res.json(result)
    })
});
app.delete('/deletebook', function (req, res) {
    db.collection('books').deleteOne({ _id: require('mongodb').ObjectId(req.query.id) }, function (err, result) {
        if (err) throw err
        res.json(result)
...
app.get('/search/:name', function (req, res) {
    db.collection('books').find({ "name": req.params.name }).toArray(function (error, result) {
        if (result.length > 0) {
...
app.post('/addbook/:cartid', function (req, res, next) {
    if (req.session.loggedIn == true) {
        ...
        db.collection('books').findOne({ _id: require('mongodb').ObjectId(req.params.cartid) }, function (error, result) {
